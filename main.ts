...
function generar_mensaje_celsius(fahrenheit: number) {
    let celsius = convertir_fahrenheit_a_celsius(fahrenheit)
    let celsius_redondeado = Math.floor(celsius * 100) / 100
...
function generar_mensaje_fahrenheit(celsius: number) {
    let fahrenheit = convertir_celsius_a_fahrenheit(celsius)
    let fahrenheit_redondeado = Math.floor(fahrenheit * 100) / 100
    return "Conversión a grados Fahrenheit:" + fahrenheit_redondeado + " °F"
}

function convertir_celsius_a_fahrenheit(celsius: number): number {
    /** Convierte grados Celsius a Fahrenheit */
    return celsius * 9 / 5 + 32
}

//  Función principal del juego
function juego(personatge: any, cofreGradosCelsius: any, cofreGradosFarenheit: any) {
